...
class LRUCache:
    def __init__(self, logger, filter_func, limit=42):
        self.filter_enabled = "-f" in sys.argv
        self.logger = logger(self, filter_func)
...
def setup_logger(obj, filter_func):
    ...
    if obj.filter_enabled:
        file_handler.addFilter(filter_func)
    ...
    if "-s" in sys.argv:
        ...
        if obj.filter_enabled:
            stdout_handler.addFilter(filter_func)
...
if __name__ == "__main__":
    cache = LRUCache(setup_logger, apply_filter, limit=3)
