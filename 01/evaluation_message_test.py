...
class TestPredictMessageMood(unittest.TestCase):
    ...
    def test_predict_message_mood(self):
        with mock.patch.object(SomeModel, 'predict') as func:
            ...
            result = predict_message_mood("This is a message", self.model)
    ...
    def test_predict_message_mood_custom_threshold(self):
        with mock.patch.object(SomeModel, 'predict') as func:
            func.return_value = 0.1
            result = predict_message_mood("This is a message", self.model, bad_threshold=0.6, good_threshold=1.7)
            self.assertEqual(result, "неуд")
            ...
            result = predict_message_mood("This is a message", self.model, bad_threshold=0.6,
                                          good_threshold=1.7)
            self.assertEqual(result, "отл")
    ...
    def test_predict_message_mood_bad_eq_good_threshold(self):
        with mock.patch.object(SomeModel, 'predict') as func:
            func.return_value = 0.5
            result = predict_message_mood("This is a message", self.model, bad_threshold=0.5, good_threshold=0.5)
            self.assertEqual(result, "норм")
